...
export async function getAdminAuditLogs(params?: Record<string, any>): Promise<TabSlice<AdminAuditLog>> {
    try {
        const axios = await getServerAxios()
        const { data } = await axios.get(`/${ADMIN_AUDIT_LOGS_ENDPOINT}`, { params: { page: 1, ...params } })
