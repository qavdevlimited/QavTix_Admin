...
import { EventStatus, getEventsListingSuspendedEvents } from "@/components/custom-utils/dropdown/resources/management-actions"
import PaginationControls from "../../tools/PaginationControl"


export default function SuspendedEventsTable() {
    
    const pagination = usePagination(mockUpcomingEvents.filter(v => v.status === "suspended"), 10)
                            ...
                            {pagination.currentItems.map((event) => (
                                <tr 
                                    ...
                                    <td className="py-4 px-4 text-right">
                                        <TableItemDropdown id={event.id} actions={getEventsListingSuspendedEvents(event.status as any, event.id)} />
                                    </td>
                                </tr>
                            ))}
                        </tbody>
