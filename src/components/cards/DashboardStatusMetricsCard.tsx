...
export default function DashboardStatusMetricsCard({ config }: DashboardStatusMetricCardsProps) {
    ...
    return (
        <div className={cn(
            "flex flex-wrap items-center gap-3 gap-y-1 w-fit max-h-20 px-4 py-3 rounded-lg border-[1.5px] transition-all duration-200 hover:shadow-md",
            styles.container
        )}>
            <h4 className={cn(space_grotesk.className, "font-bold text-xl", styles.label)}>{isMounted && config.value}</h4>

            <div className="flex flex-col justify-center">
                <div className="flex flex-col -mt-0.5">
                    <span className={cn("text-xs font-bold whitespace-nowrap", styles.label)}>
                        {isMounted && config.label}
                    </span>
